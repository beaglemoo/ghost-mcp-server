...
import { ImageUploadParams, isImageUploadParams, ImageResponse, ImageUrlUploadParams, isImageUrlUploadParams, ImageUrlUploadResponse } from '../types/index.js';
...
export const uploadImageFromUrl = async (args: unknown): Promise<{ content: { type: string; text: string }[] }> => {
  ...
  try {
    ...
    const uploadResponse = await api.images.upload(formData as any);
